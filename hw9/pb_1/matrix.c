...
void testSymmetric(void) {
    ...
    A = newMatrix(3, 3); assert(A);
    err = setElement(A, 1, 1, 1.0); assert(!err);
    err = setElement(A, 1, 2, 2.0); assert(!err);
    err = setElement(A, 1, 3, 3.0); assert(!err);
    err = setElement(A, 2, 1, 2.0); assert(!err);
    err = setElement(A, 2, 2, 4.0); assert(!err);
    err = setElement(A, 2, 3, 5.0); assert(!err);
    err = setElement(A, 3, 1, 3.0); assert(!err);
    err = setElement(A, 3, 2, 5.0); assert(!err);
    err = setElement(A, 3, 3, 6.0); assert(!err);
    printf("\nMatrix A:\n");
    err = printMatrix(A); assert(!err);
...
void testGetColumnGetRow(void) {
    ...
    err = setElement(A, 1, 2, 2.0); assert(!err);
    err = setElement(A, 1, 3, 3.0); assert(!err);
    err = setElement(A, 2, 1, 4.0); assert(!err);
    err = setElement(A, 2, 2, 5.0); assert(!err);
    err = setElement(A, 2, 3, 6.0); assert(!err);
    err = setElement(A, 3, 1, 7.0); assert(!err);
    err = setElement(A, 3, 2, 8.0); assert(!err);
    err = setElement(A, 3, 3, 9.0); assert(!err);
    printf("\nMatrix A:\n");
    err = printMatrix(A); assert(!err);
...
matrix * getColumn(matrix const * mtx, int col) {
    int row;
    matrix * result;
    if (!mtx || col < 1 || col > mtx->cols) return NULL;

    result = newMatrix(mtx->rows, 1);
    if (!result) return NULL;

    for (row = 1; row <= mtx->rows; row++) {
        setElement(result, row, 1, getE(mtx, row, col));
    }
...
matrix * getRow(matrix const * mtx, int row) {
    int col;
    matrix * result;
    if (!mtx || row < 1 || row > mtx->rows) return NULL;

    result = newMatrix(1, mtx->cols);
    if (!result) return NULL;

    for (col = 1; col <= mtx->cols; col++) {
        setElement(result, 1, col, getE(mtx, row, col));
    }
